    ...
    private static void BuildOpTable()
    {
        ...
        // Register-to-Register MOV ops, 5 cycles
        {
            ...
            foreach (var op in movOps)
            {
                ...
                _ops[op] = new CpuOp(
                    ...
                    (cpu) =>
                    {
                        var sourceValue = cpu.Reg.GetByName(opRegisters.Source);
                        cpu.Reg.SetByName(opRegisters.Destination, sourceValue);
                        return 5;
                    }
                );
            }
        ...
        // Write-To-Memory MOV Ops
        {
            ...
            foreach (var op in moveToMemOps)
            {
                ...
                _ops[op] = new CpuOp(
                    $"MOV {registers.Destination}, {registers.Source}",
                    (cpu) =>
                    {
                        // Place source register value into memory address (HL)
                        var address = cpu.Reg.HL;
                        cpu.Bus.WriteByte(address, cpu.Reg.GetByName(registers.Source));
                        return 7;
                    }
                );
            }
        ...
        // Register-from-memory MOV Ops
        {
            ...
            foreach (var op in moveFromMemOps)
            {
                ...
                _ops[op] = new CpuOp(
                    ...
                    (cpu) =>
                    {
                        var memValue = cpu.Bus.ReadByte(cpu.Reg.HL);
                        cpu.Reg.SetByName(registers.Destination, memValue);
                        return 7;
                    }
                );
            }
